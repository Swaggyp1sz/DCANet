...
def test(opt):
    ...
    # infer and evaluate performance for each model 推断和评估每个模型的性能
...
def profile(opt, lr_size, test_speed=False):
    ...
    if test_speed:
        ...
        for i in range(n_test):
            ...
            end_time = time.time()
            tot_time += (end_time - start_time) / 3  
...
if __name__ == '__main__':
   ...
   # ----------------- valid ----------------- #
        ...
        # setup paths
        base_utils.setup_paths(opt, mode='valid')
    ...
    # ----------------- profile ----------------- #
        ...
        raise ValueError(
            'Unrecognized mode: {} (train|test|profile|valid)'.format(args.mode))
