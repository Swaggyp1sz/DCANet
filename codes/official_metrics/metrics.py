...
#         for file in files:
#             if file.endswith(".png") and not file.startswith("IB"):
#                 result.append(os.path.join(root, file))
    
#     # 排序：首先按照文件名中的数字排序，如果文件名中没有数字，则放在列表前面
#     result.sort(key=lambda f: int(''.join(filter(str.isdigit, os.path.basename(f))) or -1))
#     return result
