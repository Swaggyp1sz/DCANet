...
from models.networks.ssconv import GroupBatchnorm2d, SRU, CRU, ScConv
from models.networks.DEconv import DEConv
from models.networks.A2attention import DoubleAttention
from models.networks.ACMix import ACmix
...
class SRNet(nn.Module):
    ...
    def __init__(self, in_nc=3, out_nc=3, nf=64, nb=16, upsample_func=None,
        ...
        self.conv_up = nn.Sequential(
            nn.ConvTranspose2d(nf, nf, 3, 2, 1, output_padding=1, bias=True),  #反卷积，上采样卷积。
            nn.ReLU(inplace=True),
...
class FRNet(BaseSequenceGenerator):
    ...
    def __init__(self, in_nc=3, out_nc=3, nf=64, nb=16, degradation='BD',
                 scale=4):
    ...
    def generate_dummy_input(self, lr_size):
        ...
        #lr_next = torch.rand(1, c, lr_h, lr_w, dtype=torch.float32)
    ...
    def forward(self, lr_curr, lr_prev, hr_prev):
        """
            Parameters:
                :param lr_curr: the current lr data in shape nchw        :param lr_curr：当前的 lr 数据，形状 nchw
                                                                         :param lr_prev：之前的 lr 数据，形状为 nchw
                                                                         :param hr_prev：上一个 hr 数据，形状为 nc(4h)(4w)
    ...
    def forward_sequence(self, lr_data):
        ...
        #lr_next = lr_data[:, :-1, ...].reshape(n * (t - 1), c, lr_h, lr_w)
        ...
        ret_dict = {
            ...
            #'lr_next': lr_next,  # n(t-1),c,lr_h,lr_w
    ...
    def infer_sequence(self, lr_data, device):
        ...
        for i in range(tot_frm):
            with torch.no_grad():
                ...
                # if i + 1 < tot_frm:
                #     lr_next = lr_data[i + 1: i + 2, ...].to(device)
                # else:
                # # 对于最后一帧，可以选择复制当前帧作为下一帧，或者使用其他策略
                #     lr_next = lr_curr  # 或者使用其他策略
...
class DiscriminatorBlocks(nn.Module):
    def __init__(self):
        ...
        # self.kv = nn.Conv2d(256, c, kernel_size=1, stride=1, padding=0, bias=False)
        # self.q = nn.Conv2d(256, n, kernel_size=1, stride=1, padding=0, bias=False)
        # self.proj = nn.Conv2d(n, c, kernel_size=1, stride=1, padding=0, bias=False)
