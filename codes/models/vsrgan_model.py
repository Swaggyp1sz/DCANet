...
writer = SummaryWriter('./runs/test_graph2') #定义writer 使用默认路径
...
class VSRGANModel(VSRModel):
    ...
    def train(self, data):
        ...
        if upd_D:
            ...
            """
            loss_D = dual_contrastive_loss(real_pred_D, fake_pred_D)/6  #权重为0.183 保证归一化
        ...
        real_pred_G, fake_pred_G = real_pred[0], fake_pred[0]

        gan_w = self.opt['train']['gan_crit'].get('weight', 1)
        #loss_gan_G = gan_w * self.gan_crit(fake_pred_G, True)
        loss_gan_G = gan_w * dual_contrastive_loss(fake_pred_G, real_pred_G)/10
        loss_G += loss_gan_G.detach()
